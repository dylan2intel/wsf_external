...
if [ -x "$PROJECTROOT/script/csp/opt/script/sut-info-$CSP.sh" ]; then
    ...
    vars=($("$PROJECTROOT/script/terraform/shell.sh" $CSP -v "$PROJECTROOT/script/csp:/home" -v "$PROJECTROOT/script/csp:/root" -- /opt/csp/script/sut-info-$CSP.sh $zone $rid ${profiles[@]}))
    for var1 in "${vars[@]}"; do
        echo "SUTINFO_$var1"
        eval "SUTINFO_$var1"
    done
