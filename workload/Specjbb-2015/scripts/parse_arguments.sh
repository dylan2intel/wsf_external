...
#   None
# ######################################
function getDefaultTierThreadMultiplier() {

	# defaults for most jdks and platforms (if not explictly set by user)
	default_tier_multi=( 7 0.25 1.20 )

	if [ -z "${SPECJBB_DEFAULT_T1_T2_T3_MULTIPLIERS}" ]; then

		is_zulu_jdk=$( [ "$(java --version | grep -ci zulu)" -gt 0 ] && echo "true" || echo "false" )
		read -r test_type customer < <(echo "${SPECJBB_WORKLOAD_CONFIG}" | awk -F '_' '{print $2,$3}')

		if [ "${is_zulu_jdk}" == "true" ]; then
			default_tier_multi=( 6.5 2 1 )
		elif [[ "${is_zulu_jdk}" == "false" && "${test_type}" == "crit" && "$customer" == "general" && "${PLATFORM}" == "SPR" ]]; then
			default_tier_multi=( 8.5 0.23 1.00 )
		elif [[ "${is_zulu_jdk}" == "false" && ${PLATFORM} =~ ^(SPR|ICX)$ ]]; then
			default_tier_multi=( 8.5 0.23 1.00 )
		fi

		# Higher spec machines, need to reduce thread count on SPR platforms
		online_cpu_count=$(grep -c ^processor /proc/cpuinfo)
		if [[ $online_cpu_count -ge 112 && "${PLATFORM}" == "SPR" ]]; then
			default_tier_multi=( 2 1 1 )
		fi
	else
		IFS=',' read -a default_tier_multi <<< "${SPECJBB_DEFAULT_T1_T2_T3_MULTIPLIERS}"
	fi

	echo "${default_tier_multi[0]}|${default_tier_multi[1]}|${default_tier_multi[2]}"
}
