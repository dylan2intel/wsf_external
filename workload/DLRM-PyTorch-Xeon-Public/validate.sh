...
WORKLOAD=$(echo ${OPTION}|cut -d_ -f1-4)
TOPOLOGY=${TOPOLOGY:-dlrm}
FUNCTION=$(echo ${OPTION}|cut -d_ -f5)
MODE=$(echo ${OPTION}|cut -d_ -f6)
...
if [ "$CASE_TYPE" == "pkm" ]; then
    EVENT_TRACE_PARAMS="roi,Testing for inference only,Complete..."
fi
...
# Docker Setting
if [ "$MODE" == "accuracy" ] && [ "$FUNCTION" == "inference" ]; then
